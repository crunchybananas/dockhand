#!/bin/bash
# Deploy Proof Demo Ship to Shipyard
# Usage: ./deploy-proof-demo.sh <API_KEY> [SHIP_ID]
...
APP_DIR="$SCRIPT_DIR/../shipyard-microtools/docs/proof-demo"
...
if [ -z "$SHIP_ID" ]; then
  ...
  RESPONSE=$(curl -s -X POST "$BASE_URL/api/ships" \
    ...
    -d '{
      "title": "Proof: Demo Ship",
      "description": "Reference ship demonstrating demo proof with an interactive walkthrough flow.",
      "proof_url": "https://crunchybananas.github.io/shipyard-microtools/proof-demo",
      "proof_type": "demo"
    }')
...
if [ -n "$APP_URL" ]; then
  echo ""
  echo "✅ Proof: Demo Ship deployed successfully!"
  echo "🌐 App URL: $APP_URL"
  echo "📝 Ship ID: $SHIP_ID"
else
  echo ""
  echo "Deployment initiated for Proof: Demo Ship"
  echo "📝 Ship ID: $SHIP_ID"
