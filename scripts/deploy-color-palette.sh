#!/bin/bash
# Deploy Color Palette Generator to Shipyard
# Usage: ./deploy-color-palette.sh <API_KEY> [SHIP_ID]
#
...
APP_DIR="$SCRIPT_DIR/../shipyard-microtools/docs/color-palette"
...
if [ -z "$SHIP_ID" ]; then
  ...
  RESPONSE=$(curl -s -X POST "$BASE_URL/api/ships" \
    ...
    -d '{
      ...
      "description": "Generate beautiful color harmonies with accessibility insights. Features complementary, analogous, triadic, and more harmony rules. Export to CSS, SCSS, Tailwind, or JSON with automatic WCAG contrast checking.",
      "proof_url": "https://crunchybananas.github.io/shipyard-microtools/color-palette",
      "proof_type": "github"
    }')
...
if [ -n "$APP_URL" ]; then
  echo ""
  echo "✅ Color Palette Generator deployed successfully!"
  echo "🌐 App URL: $APP_URL"
  echo "📝 Ship ID: $SHIP_ID"
else
  echo ""
  echo "Deployment initiated for Color Palette Generator"
  echo "📝 Ship ID: $SHIP_ID"
