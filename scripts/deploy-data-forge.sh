#!/bin/bash
# Deploy Data Forge to Shipyard
# Usage: ./deploy-data-forge.sh <API_KEY> [SHIP_ID]
#
...
APP_DIR="$SCRIPT_DIR/../shipyard-microtools/docs/data-forge"
...
if [ -z "$SHIP_ID" ]; then
  ...
  RESPONSE=$(curl -s -X POST "$BASE_URL/api/ships" \
    ...
    -d '{
      "title": "Data Forge",
      "description": "Generate realistic mock data for testing and development. Create customizable datasets with 50+ data types including names, emails, addresses, companies, and more. Export to JSON, CSV, SQL, XML, or YAML. Perfect for prototyping and testing.",
      "proof_url": "https://crunchybananas.github.io/shipyard-microtools/data-forge",
      "proof_type": "github"
    }')
...
if [ -n "$APP_URL" ]; then
  echo ""
  echo "✅ Data Forge deployed successfully!"
  echo "🌐 App URL: $APP_URL"
  echo "📝 Ship ID: $SHIP_ID"
else
  echo ""
  echo "Deployment initiated for Data Forge"
  echo "📝 Ship ID: $SHIP_ID"
