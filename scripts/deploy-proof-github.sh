#!/bin/bash
# Deploy Proof GitHub Ship to Shipyard
# Usage: ./deploy-proof-github.sh <API_KEY> [SHIP_ID]
...
APP_DIR="$SCRIPT_DIR/../shipyard-microtools/docs/proof-github"
...
if [ -z "$SHIP_ID" ]; then
  ...
  RESPONSE=$(curl -s -X POST "$BASE_URL/api/ships" \
    ...
    -d '{
      "title": "Proof: GitHub Ship",
      "description": "Reference ship demonstrating GitHub proof with a repository readiness checklist.",
      "proof_url": "https://github.com/crunchybananas/shipyard-microtools",
      "proof_type": "github"
    }')
...
if [ -n "$APP_URL" ]; then
  echo ""
  echo "✅ Proof: GitHub Ship deployed successfully!"
  echo "🌐 App URL: $APP_URL"
  echo "📝 Ship ID: $SHIP_ID"
else
  echo ""
  echo "Deployment initiated for Proof: GitHub Ship"
  echo "📝 Ship ID: $SHIP_ID"
